...
-- 1. Criar tabela justificativas_os se não existir
CREATE TABLE IF NOT EXISTS public.justificativas_os (
...
-- 2. Criar tabela auditoria_login se não existir
...
-- 3. Adicionar coluna tempo_tolerancia_pausa na tabela configuracoes se não existir
ALTER TABLE public.configuracoes 
ADD COLUMN IF NOT EXISTS tempo_tolerancia_pausa INTEGER DEFAULT 120;

-- 4. Criar índices para performance
CREATE INDEX IF NOT EXISTS idx_justificativas_os_os_id ON public.justificativas_os(os_id);
...
-- 6. Criar políticas RLS (usando DROP IF EXISTS para evitar conflitos)
DROP POLICY IF EXISTS "Enable read access for all users" ON public.justificativas_os;
CREATE POLICY "Enable read access for all users" ON public.justificativas_os FOR SELECT USING (true);

DROP POLICY IF EXISTS "Enable insert for authenticated users" ON public.justificativas_os;
...
DROP POLICY IF EXISTS "Enable update for authenticated users" ON public.justificativas_os;
CREATE POLICY "Enable update for authenticated users" ON public.justificativas_os FOR UPDATE USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Enable delete for authenticated users" ON public.justificativas_os;
...
-- 7. Inserir dados de exemplo na auditoria
...
-- 8. Inserir configuração de tempo de tolerância se não existir
